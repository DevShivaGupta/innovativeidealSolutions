...
import { WebDevelopment } from "../../helper/Data";
import ServiceSubCard from "./ServiceSubCard";
import webprocess from "../../images/our-web.png";
    ...
    <div>
        ...
        <Container className={classes.chooseContainer}>
          <div className={classes.chooseWrapper}>
            <Text component="span" className={classes.choosehead}>
              {WebDevelopment.reasonToChoose.head}
            </Text>
            <Text component="div" className={classes.serviceDetails}>
              {WebDevelopment.reasonToChoose.list.map((item, i) => {
      ...
      <Fade bottom cascade>
        <Container size={"70%"} className={classes.allServiceCard} mt={60}>
          <Grid>
            {WebDevelopment.services.map((item) => (
      ...
      <Fade bottom>
        <Container mt={40} size={"60%"} className={classes.allServiceCard}>
          <img src={webprocess} alt="web dev" width="100%" />
        </Container>
        <Container className={classes.chooseContainer}>
          <Text component="span" className={classes.choosehead}>
            {WebDevelopment.benefits.head}
          </Text>
          <Text component="div" className={classes.serviceDetails}>
            <Text mb={20} className={classes.serviceDetails}>
              {WebDevelopment.benefits.details}
            </Text>
            {WebDevelopment.benefits.list.map((item, i) => {
              return <li key={i}>{item}</li>;
            })}
          </Text>
        </Container>
